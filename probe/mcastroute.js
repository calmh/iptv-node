...
exports.testMembershipDifference = function () {
        var wanted = [ '239.193.46.32', '239.193.46.33', '239.193.46.34' ];
        var actual = [ '239.193.46.32', '239.193.46.34', '239.193.46.35' ];
        var diff = exports.membershipDifference(actual, wanted);
        assert.ok(_.isEqual(diff.join, [ '239.193.46.33' ]));
        assert.ok(_.isEqual(diff.leave, [ '239.193.46.35' ]));
}

// Join specified multicast group.
exports.join = function (group, interface) {
        exec('smcroute -j ' + interface + ' ' + group);
}

// Leave specified multicast group.
exports.leave = function (group, interface) {
        exec('smcroute -l ' + interface + ' ' + group);
}
